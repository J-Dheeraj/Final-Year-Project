...
| PoV (proof of vulnerability) | `dynamically_confirmed` + `execution_log` on `ExploitArtifacts` | Real — a genuine subprocess exit code from a genuine HTTP exchange or compiled-harness run, not a static claim. This is the pipeline's strongest CRS-alignment point. o2lab's own README calls the same principle "zero-hallucination verification" ("every candidate PoV input is executed... and kept only if the sanitizer fires") — same discipline, independently named by a real finalist team. |
